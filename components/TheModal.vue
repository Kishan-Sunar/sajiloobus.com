...
<template>
    ...
    >
        <Transition
            enter-from-class="transition duration-400 opacity-0"
            enter-to-class="transition duration-300 opacity-100"
            leave-from-class="transition duration-400 opacity-100"
            leave-to-class="transition duration-300 opacity-0"
        >
            <div
                v-if="show"
                @click="modalStore.toggleModal('none')"
                class="fixed top-0 left-0 w-full bottom-0 bg-black/60"
            ></div>
        </Transition>
        <div class="px-4 sm:px-0 h-full flex justify-center items-center">
            <Transition
                enter-from-class="transition duration-400 scale-90 opacity-0"

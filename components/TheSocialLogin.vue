...
const handleOnSuccess = async (response: AuthCodeFlowSuccessResponse) => {
    try {
        await usePassengerStore().googleSignIn(response);
...
<template>
    <div class="flex flex-col gap-y-4">
        <div class="relative gap-x-5 flex items-center justify-center">
            <span
                class="bg-gradient-to-l rounded-full from-zinc-200 h-0.5 w-full"
            ></span>
            <span class="font-medium text-zinc-600 text-sm whitespace-nowrap"
                >or continue</span
            >
            <span
                class="bg-gradient-to-r rounded-full from-zinc-200 h-0.5 w-full"
            ></span>
        </div>

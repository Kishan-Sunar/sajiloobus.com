...
busStore.getData();
const { data, selected, searchData } = storeToRefs(busStore);
...
<template>
                    ...
                    <div class="pb-4 border-b">
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-y-2 gap-x-4">
                            <div class="flex flex-col gap-y-1">
                                ...
                                <span class="text-base font-medium">{{ item.bus_no }}</span>
                            </div>
                            <div class="flex flex-col gap-y-1">
                                <h2 class="text-sm font-medium text-slate-500">
                                    Name
                            ...
                            <div class="flex justify-end gap-y-1">
                                <SharedActionButton
                                    @action="() => (busStore.selectData(item), modalStore.toggleModal('edit'))"
                                    title="Edit">
                                    <IconEdit class="h-5" />
                                </SharedActionButton>
                                <SharedActionButton
                                    @action="() => (busStore.selectData(item), modalStore.toggleModal('delete'))"
                                    title="More options">
    ...
    <ModalsBusAdd :show="currentModal == 'add'" />
    <ModalsBusEdit :show="currentModal == 'edit'" />
    <ModalsBusDelete :show="currentModal == 'delete'" />
</template>